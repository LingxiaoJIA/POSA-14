	...
	static class MessengerHandler extends Handler {
		...
		public void handleMessage(Message msg) {
			...
			if (activity != null) {
				...
				if (data != null) {
					outputPath = data.getString(DownloadUtils.PATHNAME_KEY);
				} else
					return;
				if (outputPath != null) {
					activity.displayBitmap(outputPath);
				} else
					return;
			}
		}
	...
	public void runService(View view) {
		...
		switch (view.getId()) {
			case R.id.intent_service_button:
				...
				intent = DownloadIntentService.makeIntent(this, handler, url);
				startService(intent);
			...
			case R.id.thread_pool_button:
				...
				intent = ThreadPoolDownloadService.makeIntent(this, handler,
				        url);
				startService(intent);
				break;
